'''
Description: A class to monitor the cgroup stats of a process
How to use:
...
class CGroupMonitor(threading.Thread):
    ...
    def get_cpu_usage(self, interval=0.1):
        ...
        Parameters:
        - interval (float): The interval to calculate the CPU usage. Default 0.1s
    ...
    def get_memory_usage_percent(self, decimal_places=2):
        ...
        Returns:
        - memory_usage_percent (float): The memory usage percentage of the process
        '''
    ...
    def start_monitor(self):
        ...
        The monitor will keep a track of the CPU and Memory usage of the entire system
    ...
    def stop_monitor(self):
        '''
        Ends the monitor and returns the average CPU and Memory usage of the process

        Returns:
        - cpu_usage (float): The average CPU usage of the process (in percentage)
        - memory_usage (float): The average Memory usage of the process (in percentage)
        '''
        ...
        if len(self.cpu_usage_percent_list) > 0 and len(
                self.memory_usage_percent_list):
    ...
    def constant_monitoring(self):
        ...
        while not self._stop_event.is_set():
            ...
            if (time.time_ns() - self.monitor_start_time) > 1000000000:
                # print(
                #     f"CPU Usage: {self.get_cpu_usage_percent()}% Memory Usage: {self.get_memory_usage_percent()}%"
                # )
                self.monitor_start_time = time.time_ns()
